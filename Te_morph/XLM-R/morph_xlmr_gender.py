# -*- coding: utf-8 -*-
"""Morph_xlmr_gender.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/19gRjtEFmIC80T6wxpBXWIkFCVlnzboct
"""
...
for i in range(len(df_cleaned)):
  if(df_cleaned["gender"][i]=="fm" ):
    df_cleaned["gender"][i]="mf"
...
for i in range(len(df_cleaned["tokens"])):
  if(df_cleaned["tokens"][i]!="."):
    if(df_cleaned["gender"][i]=="m" or df_cleaned["gender"][i]=="mf" or df_cleaned["gender"][i]=="f" or df_cleaned["gender"][i]=="fn" or df_cleaned["gender"][i]=="n" or df_cleaned["gender"][i]=="any"):
      sentence.append(df_cleaned["tokens"][i])
      gl.append(df_cleaned["gender"][i])
  else:
    # sentence.append(df_cleaned["tokens"][i])
    # gl.append(df_cleaned["gender"][i])
    mosentences.append(sentence)
    gender_labels.append(gl)
    sentence=[]
    gl=[]

flattened_list = [item for sublist in gender_labels for item in sublist]
...
# Example list
my_list = flattened_list

# Count the occurrences of each item in the list
item_counts = Counter(my_list)

# Get the distinct items and their count
distinct_items = list(item_counts.keys())
distinct_items_count = list(item_counts.values())

# Print the distinct items and their count
for item, count in zip(distinct_items, distinct_items_count):
...
# model = AutoModel.from_pretrained("jplu/tf-xlm-r-ner-40-lang")
...
tokenized_texts_and_labels = [
    ...
    for sent, labs in zip(mosentences, gender_labels)
]
...
tags = pad_sequences([[tag2idx.get(l) for l in lab] for lab in gender_labels],
...
# Load the XLM-R model for token classification
